...
public:
  ...
  bool make_move(const Move &move) {
    ...
    case PROMO_QUEEN:
      ...
      squares[from] = (current_color ? Piece(2 * move.promotion_piece() + 1)
                                     : Piece(2 * move.promotion_piece()));
  ...
  /// \param square
  /// \return
  Piece at(Square square) const { return squares[square]; }
...
private:
  ...
  /// \param move
  /// \return
  bool is_legal(const Move &move) const { return true; }
};
} // namespace BBD