...
/*
This is usually a very good basic starting evaluation. Taken from here:
...
inline int board_evaluation(const Board &board)
{
    ...
    for (PieceType p = PieceTypes::PAWN; p <= PieceTypes::KING; p++)
    {
        ...
        Bitboard b = board.get_piece_bitboard(Colors::WHITE, p);
        while (b)
        {
        ...
        mg += (white_score - black_score) * mg_value[p];
        eg += (white_score - black_score) * eg_value[p];
        phase += (white_score + black_score) * phase_value[p];
    }
